...
public class Modificar_contrasenia extends AppCompatActivity implements View.OnClickListener{
    ...
    public void onClick(View v) {
        if(v.getId()== R.id.confirmar)
            {  if(nueva_contrasenia.getText().toString().trim().equals(repetir_contrasenia.getText().toString().trim())) {
                if(nueva_contrasenia.getText().toString().trim().length()>=6){
                    ...
                    servicio.execute(getString(R.string.servidor) + "frmTaxi.php?opcion=actualizar_contrasenia", "1", perfil.getString("ci", ""), contrasenia_antigua.getText().toString(), nueva_contrasenia.getText().toString());
                }else
                {
...
//Enviamos un correo m¡para confirmar la modificacion de su cuenta..
    ...
    {
        try {
            AlertDialog.Builder builder = new AlertDialog.Builder(Modificar_contrasenia.this);
